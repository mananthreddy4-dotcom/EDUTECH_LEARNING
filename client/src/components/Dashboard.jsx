...
function Dashboard({ user, onLogout }) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-900 to-indigo-950">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <button
                      onClick={() => {
                        ...
                        setSuccess('Teacher ID copied to clipboard!');
                        setTimeout(() => setSuccess(''), 2000);
                      }}
                      className="px-2 py-0.5 bg-violet-500/20 hover:bg-violet-500/30 text-violet-300 text-xs rounded border border-violet-500/30 transition-all font-mono"
                      title="Click to copy"
                    >
                      {user.id.slice(0, 8)}... 📋
                    </button>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Teacher ID Card - Only for Teachers */}
        {user.role === 'teacher' && (
          <div className="bg-gradient-to-r from-violet-500/10 to-purple-500/10 backdrop-blur-xl border border-violet-500/30 rounded-2xl p-6 mb-8 shadow-xl">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div>
                <h3 className="text-lg font-semibold text-slate-50 mb-2 flex items-center gap-2">
                  🎓 Your Teacher ID
                </h3>
                <p className="text-sm text-slate-300 mb-3">
                  Share this ID with your students so they can create their accounts
                </p>
                <div className="flex items-center gap-3">
                  <code className="px-4 py-2 bg-slate-900/50 border border-violet-500/30 rounded-lg text-violet-300 font-mono text-sm">
                    {user.id}
                  </code>
                  <button
                    onClick={() => {
                    ...
                    className="px-4 py-2 bg-gradient-to-r from-violet-500 to-purple-500 hover:from-violet-600 hover:to-purple-600 text-white font-semibold rounded-lg shadow-lg hover:shadow-xl transition-all duration-200 flex items-center gap-2 focus:ring-2 focus:ring-violet-500 focus:outline-none"
                  >
                    📋 Copy ID
                  </button>
                </div>
              </div>
              <div className="text-sm text-slate-400 bg-slate-900/30 rounded-lg p-3 border border-white/10">
                <p className="font-semibold text-slate-300 mb-1">📝 Instructions for Students:</p>
                <ol className="list-decimal list-inside space-y-1 text-xs">
                  <li>Go to the signup page</li>
                  <li>Select "Student" role</li>
                  <li>Paste this Teacher ID</li>
